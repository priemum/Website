...
module.exports = async (client, member) => {
       ...
       const welcomeChannel = member.guild.channels.cache.find(chnl => chnl.name === "┊greetings");
  ...
  if (theMember.bot) { 
      ...
      let suffixed = end == 1 ? count : end == 2 ? count : end == 3 ? count : count 
       ...
       const botJoinMessage = new MessageEmbed()
        ...
        welcomeChannel.send(botJoinMessage)
         ...
         member.guild.members.fetch(client.users.cache.find(u => u.id === member.id)).then(bot => {
      ...
      let rules = await member.guild.channels.cache.find(c => c.id === "748977820784394244");
      let subChan = await member.guild.channels.cache.find(c=> c.id === "762752158809587732");

       const welcomeMessage = new MessageEmbed()
             .setTitle("➕ A Member has Joined ➕")
             .setDescription(`Welcome to Paradise <@${member.user.id}>, Make sure you check out the ${rules} channel and enjoy your stay.`)
             .addField("Member", `${member.user.username}`, true)
             .addField("Discrim", `${member.user.discriminator}`, true)
             .addField("New Member Count", `${suffixed} Members`, true)
             .addField('Joined Discord', `${moment(member.user.createdAt).toString().substr(0, 15)}\n(${moment(member.user.createdAt).fromNow()})`, true)
             .addField('Joined Server', `${moment(member.user.joinedAt).toString().substr(0, 15)}\n(${moment(member.user.joinedAt).fromNow()})`, true)
             .addField('Subscribe to Updates', '`p>sub` or Visit: ' + `${subChan}`, true)    
             .setThumbnail(theMember.displayAvatarURL())

       welcomeChannel.send(welcomeMessage)

       member.guild.members.fetch(client.users.cache.find(u => u.id === member.id)).then(user => {

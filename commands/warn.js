...
module.exports.run = async (client, message, args) => {
    ...
    try{
        ...
        if(warnedUser.id === message.author.id) return message.reply('Why would i let you do that!!'); 

        let theUser = await user_warns.findOne({ userID: warnedUser.id });

        let modLogs = message.guild.channels.cache.find(c => c.name === 'mod-logs');
        ...
        let noPerms = new MessageEmbed()
            ...
            .setColor('#7289DA')
            .setDescription('You dont have perms to use this command, If you believe this is a mistake please contact any of our Onwers or Head Admins for assistance.')
    ...
    if (reasonForWarn.length > 100) return message.channel.send('The reason should not be more then 100 Characters.');
       ...
       if (theUser.numberOfWarns > 3) {
            ...
            message.guild.members.fetch(message.client.users.cache.find(u => u.id === warnedUser.id)).then(user => {
        ...
        if (theUser.numberOfWarns > 6) {
           ...
           let kickEmbed = new MessageEmbed()
            ...
            .setDescription(`${warnedUser} has been kicked automatically for receiving a total of 6 Warns`)
            .setColor('#7289DA') 
            .setTimestamp()

           let dmEmbed = new MessageEmbed()
            .setTitle('Whoops, To many Warns!!')
            .setDescription(`$You have been automatically kicked for receiving a total of 6 Warns, Feel free to read our rules and rejoin.`)
            .setColor('#7289DA')
            .addField('Server Rules', '[Click Here](https://paradisebots.net/serverrules)', true)
            .addField('Re-Join the Server', '[Click Here](https://paradisebots.net/join)', true) 
            .setTimestamp()
        ...
        if (theUser.numberOfWarns > 9) {
           ...
           let banEmbed = new MessageEmbed()
            ...
            .setDescription(`${warnedUser} has been banned automatically for receiving a total of 9 Warns`)
            .setColor('#7289DA') 
            .setTimestamp()
        ...
        errorEmbed.setDescription("If this issue continues please contact our Dev Team")

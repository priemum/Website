...
mongoose.connect("mongodb://admin:XclUd5hhWSJDU4Nl@SG-ParadiseBotList-35989.servers.mongodirector.com:27017/admin" , {
...
module.exports = (client, message) => {
    ...
    if (message.content === `<@${client.user.id}>` || message.content === `<@!${client.user.id}>`) return message.channel.send(pingMsg)
    ...
    let ownerEmbed = new MessageEmbed()
           ...
           ownerEmbed.setDescription("Okay noob You dont have permission to use this command!!")
...
let higherStaffMsg = new MessageEmbed()
           ...
           higherStaffMsg.setDescription("Okay noob You dont have permission to use this command!!")

    if(cmd.requirements.higherOnly && !client.config.higherStaff.includes(message.author.id))

...
const botAvatar = client.users.cache.get(botToVerify.botid).displayAvatarURL()
...
message.guild.members.fetch(message.client.users.cache.find(u => u.id === botToVerify.owner)).then(owner => {

 owner.send(e3)

 })

message.guild.members.fetch(message.client.users.cache.find(u => u.id === botToVerify.botid)).then(bot => {
...
category: "Bot List",
aliases: ["deny"],
description: "Remove/Delete the provided bot from our site, Can be resubmitted.",
example: "``remove <@bot>``"
}

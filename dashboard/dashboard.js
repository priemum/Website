...
const converter = new showdown.Converter(); converter.setOption('tables', 'true');
...
app.get("/github", async (req, res, next) => {
  ...
  */
  ...
  app.get("/apps/staff", checkAuth, async (req, res, next) => {
    renderTemplate(res, req, "applications/staff-app.ejs", { alert: null, error: null });
  })

  app.post("/apps/staff", checkAuth, async (req, res, next) => {
    let check = await staff_app.findOne({ userID: req.user.id });
    if (check) {
      renderTemplate(res, req, "applications/staff-app.ejs", { alert: null, error: "You are already submitted!" });
    } else {
      if (req.body.submit) {
        await new staff_app({
        ...
        let r = client.guilds.cache.get(config.guildid).roles.cache.find(r => r.id === '748977820776267871');
        await r.setMentionable(true)
        await client.guilds.cache.get(config.guildid).channels.cache.get("762752189402972190").send(`Hey ${r} | <@${req.user.id}> (\`${req.user.username}\`) submitted for staff app!`)
        renderTemplate(res, req, "applications/staff-app.ejs", { alert: "Your staff  app has been sent to our staff team!", error: null });
      }
    };
  })
  ...
  app.get("/apps/partner", checkAuth, async (req, res, next) => {
    renderTemplate(res, req, "applications/partner-app.ejs", { alert: null, error: null });
  })

  app.post("/apps/partner", checkAuth, async (req, res, next) => {
    let check = await partner_app.findOne({ userID: req.user.id });
    if (check) {
      renderTemplate(res, req, "applications/partner-app.ejs", { alert: null, error: "You are already submitted!" });
    } else {
      if (req.body.submit) {
        ...
        let r = client.guilds.cache.get(config.guildid).roles.cache.find(r => r.id === '748977820776267871');
        await r.setMentionable(true)
        await client.guilds.cache.get(config.guildid).channels.cache.get("762752187692351538").send(`Hey ${r} | <@${req.user.id}> (\`${req.user.username}\`) submitted for partner program!`)
        renderTemplate(res, req, "applications/partner-app.ejs", { alert: "Your partner app has been sent to our staff team!", error: null });
      }
    }
  })
...
*/
  ...
  app.get('/api/v1/users/:userID', async (req,res) => {
    ...
    if (!users) return res.status(404).send(JSON.stringify({ ParadiseAPI: 'User not found!' }));
  ...
  app.get("/api/v1/bots/:botid", async (req, res) => {
    ...
    const botsVotedUsers = await VOTES.find({ clientid: req.params.botid}).limit(10)
    
    if (!bots) return res.status(404).send(JSON.stringify({ ParadiseAPI: "Bot not found!" }));

    if (bots.status === "pending") return res.status(403).send(JSON.stringify({ ParadiseAPI: "Bot is not approved." }));
    ...
    let data = {
         ...
         vanityURL: bots.vanity === 'none' || !bots.is_certified ? "None" : bots.vanity,
         owner: bots.owner,
         ...
         server: bots.serverlink === 'https://discord.gg/Cqy99Pt' ? "None" : bots.serverlink,
         website: bots.website === 'https://paradisebots.net/' ? "None" : bots.website,
         github: bots.github.includes('IE:') || bots.github === 'https://github.com/' ? "None" : bots.github,
         donate: bots.donate === 'https://www.patreon.com/ParadiseBots' || bots.donate === 'https://paypal.me/' ? "None" : bots.donate,
         tags: bots.tags || "None",
         ...
         dislikes: bots.dislikes.size === 0 ? "Bot has No Dislikes" : bots.dislikes.size
     } 
        res.status(200).send(JSON.stringify(data))
  });
  ...
  app.post("/api/v1/bot/:botid", async (req, res, next) => {
    ...
    if (!auth) return res.status(403).send(JSON.stringify({ 
      ParadiseAPI: "Authorization header not found, Go generate a auth token for your bot or make sure it's valid!",
      error: true,
      status: "403"
    }));
    let servers = req.body.server_count
    if (!servers) return res.status(404).send(JSON.stringify({ 
      ParadiseAPI: "Can't find server_count.",
      error: true,
      status: "404"
    }));
    servers = parseInt(servers);
    if (!servers) return res.status(400).send(JSON.stringify({ 
      ParadiseAPI: "server_count not integer.", 
      error: true, 
      status: "400"
    }));
    BOTS.findOne({ botid: req.params.botid }, (err, bot2) => {
      if (!bot2) return res.status(404).send(JSON.stringify({ 
        ParadiseAPI: "Bot not found!",
        error: true,
        status: "404"
      }));
      if (!bot2.auth) return res.status(403).send(JSON.stringify({ 
        ParadiseAPI: "Authorization header not found, Go generate a auth token for your bot or make sure it's valid!",
        error: true,
        status: "403"
      }));
      if (bot2.auth !== auth) {
        ...
        BOTS.findOne({ botid: req.params.botid }, async (err, bot) => {
          if (!bot) return res.status(404).send(JSON.stringify({ 
            ParadiseAPI: "Bot not found.",
            error: true,
            status: "404"
          }));
          if (bot.status === "pending") return res.status(403).send(JSON.stringify({ 
          ...
          if (req.body.shard_count) {
            if (!parseInt(req.body.shard_count)) return res.status(400).send(JSON.stringify({ 
          ...
          if (ratelimit.has(req.params.botid + "stats")) return res.status(429).send(JSON.stringify({ 
            ParadiseAPI: "Your are being ratelimited, 1 request per 5 mins.",
            error: true,
            status: "429"
          }));
          await bot.save()
  ...
  app.get("/search=:value", async (req, res, next) => {
    ...
    renderTemplate(res, req, "bots/search.ejs", { isCertifiedBot: bots.certifiedBot === 'certified' ? true : false, bots: bots, alert: null, error: null });
  });
  ...
  app.get("/panel", checkAuth, async (req, res, next) => {
   ...
   /*if(!is_staff(req.user.id)) {
      ...
      renderTemplate(res, req, "panel.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: null, error: null, reportbug: bugres, partners: partners, staff_apps: staff_apps });
    }
  });
    ...
    if (!config.owners.includes(req.user.id)) {
      ...
      if (req.body.reject) {
        if (!client.guilds.cache.get(config.guildid).members.cache.get(req.body.reject)) {
          return renderTemplate(res, req, "panel.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: null, error: "Bot is not at server!", partners: partners, staff_apps: staff_apps });
        }
        BOTS.findOne({ botid: req.body.reject }, async (err, bot) => {
          if (!bot) {
            return renderTemplate(res, req, "panel.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: null, error: "Error occured while rejecting bot!", partners: partners, staff_apps: staff_apps });
          } else {
            if (bot.status == "denied") return renderTemplate(res, req, "panel.ejs", { webAdmin: webAdmin, partners: partners, reportbug: bugres, bots: penbotlist, alert: null, error: "Someone already rejected the bot faster than you!", staff_apps: staff_apps });
            var deniedMsg = new MessageEmbed()
              .setTitle('Bot Denied')
              .setColor('#7289DA')
              .setDescription("Whoops, A bot has been rejected, Please make sure you read our rules before you resubmit")
            ...
            client.guilds.cache.get(config.guildid).channels.cache.get(config.botlogs).send(deniedMsg)
            ...
            return renderTemplate(res, req, "panel.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: "Success the bot has been denied!", error: null, partners: partners, staff_apps: staff_apps });
          }
        })
      }
      if (req.body.accept) {
          ...
          if (!bot) {
            ...
            if (!client.guilds.cache.get(config.guildid).members.cache.get(bot.owner)) {
              var autoDeny = new MessageEmbed()
                ...
                .setColor('#7289DA')
                .setDescription("Whoops, Looks like the owner left the server, The bot will now be removed from our list")
              ...
              client.guilds.cache.get(config.guildid).channels.cache.get(config.botlogs).send(autoDeny)
              ...
              penbotlist = await BOTS.find({ status: "pending" }, { _id: false, auth: false }).sort([['descending']])
              penbotlist.filter(bot => bot)
              return renderTemplate(res, req, "panel.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: null, error: "Bot owner is not at server!, bot got auto rejected", partners: partners, staff_apps: staff_apps });
            }
            if (bot.status == "approved") return renderTemplate(res, req, "panel.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: null, error: "Someone already approved the bot faster than you!", partners: partners, staff_apps: staff_apps });
            var approved = new MessageEmbed()
              ...
              .addField("Bot Page", `[View it Here](https://paradisebots.net/bots/${req.body.accept})`)
              .setTimestamp()
            let theOwner = client.guilds.cache.get(config.guildid).members.cache.get(bot.owner);
            client.guilds.cache.get(config.guildid).channels.cache.get(config.botlogs).send(approved)
            ...
            penbotlist = await BOTS.find({ status: "pending" }, { _id: false, auth: false }).sort([['descending']])
            penbotlist.filter(bot => bot)
            return renderTemplate(res, req, "panel.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: "Success the bot has accepted!", error: null, partners: partners, staff_apps: staff_apps });
          }
        })
      }
      let alertmsg = "";
      ...
      if (req.body.acceptbug) {
        ERRORS.findOneAndDelete({ userID: req.body.acceptbug }, async (err, bug) => {
          if (!bug) {
            ...
            bugres.filter(bug => bug)
            return renderTemplate(res, req, "panel.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: alertmsg, error: errormsg, partners: partners, staff_apps: staff_apps });
          } else {
      ...
      if (req.body.accept_partner) {
        let user_partner = await partner_app.findOne({ userID: req.body.accept_partner });
        ...
        if (!user_partner) {
          ...
          partners.filter(app => app)
          renderTemplate(res, req, "panel.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: alertmsg, error: errormsg, partners: partners, staff_apps: staff_apps });
        } else {
          alertmsg = `Successful accepted the partner!`
          await client.guilds.cache.get(config.guildid).channels.cache.get("762752184944689192").send(`<@${user_partner.userID}> (\`${user_partner.username}\`) partner app has been approved! `)
          let user = await client.users.fetch(user_partner.userID);
          await user.send("Your partner app at paradise bot list has been approved").catch(() => { });
          let member = client.guilds.cache.get(config.guildid).members.cache.get(user_partner.userID);
          let role = client.guilds.cache.get(config.guildid).roles.cache.get("749862856035401807");
      ...
      if (req.body.deny_partner) {
        let user_partner = await partner_app.findOne({ userID: req.body.deny_partner });
        ...
        if (!user_partner) {
          ...
          await client.guilds.cache.get(config.guildid).channels.cache.get("762752184944689192").send(`<@${user_partner.userID}> (\`${user_partner.username}\`) partner app has been denied! by: <@${req.user.id}>, Please contact this Staff Member for more info.`)
          let user = await client.users.fetch(user_partner.userID);
          await user.send(`Your partner app at paradise bot list has been denied by: <@${req.user.id}>(${req.user.username}), Please contact this Staff Member for more info`).catch(() => { });
          await user_partner.deleteOne();
          partners = await partner_app.find({}, { _id: false, auth: false })
          partners.filter(app => app)
          return renderTemplate(res, req, "panel.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: alertmsg, error: errormsg, partners: partners, staff_apps: staff_apps });
      ...
      if (req.body.accept_staff) {
        let new_staff = await staff_app.findOne({ userID: req.body.accept_staff });
        ...
        if (!new_staff) {
          ...
          await client.guilds.cache.get(config.guildid).channels.cache.get("762752184944689192").send(`<@${new_staff.userID}> (\`${new_staff.username}\`) staff app has been approved with **${client.guilds.cache.get(client.config.guildid).roles.cache.get(new_staff.position).name}** position!`)
          let user = await client.users.fetch(new_staff.userID);
          await user.send("Your staff app at paradise bot list has been approved").catch(() => { });
          let member = client.guilds.cache.get(config.guildid).members.cache.get(new_staff.userID);
          let role = client.guilds.cache.get(config.guildid).roles.cache.get(new_staff.position);
          await member.roles.add(role);
          await new_staff.deleteOne();
          staff_apps = await staff_app.find({}, { _id: false, auth: false })
          staff_apps.filter(app => app)
      ...
      if (req.body.deny_staff) {
        ...
        if (!staff) {
          ...
          await client.guilds.cache.get(config.guildid).channels.cache.get("762752184944689192").send(`<@${staff.userID}> (\`${staff.username}\`) staff app has been denied! by: <@${req.user.id}>, Please contact this Staff Member for more info.`)
          let user = await client.users.fetch(staff.userID);
          await user.send(`Your staff app at paradise bot list has been denied by: <@${req.user.id}>(${req.user.username}), Please contact this Staff Member for more info`).catch(() => { });
          await staff.deleteOne();
          staff_apps = await staff_app.find({}, { _id: false, auth: false })
          staff_apps.filter(app => app)
          return renderTemplate(res, req, "panel.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: alertmsg, error: errormsg, partners: partners, staff_apps: staff_apps });
...
app.post("/add", checkAuth, async (req, res, next) => {
    ...
    //Guild Error
    else if (!guild.members.cache.get(req.user.id)) errormsg = "You are not in our server!"
    //Short Description
    else if (req.body.shortdesc.length > 150) errormsg = "Short Description: Short description can't be more than 150 charactars"
    else if (req.body.shortdesc.length < 10) errormsg = "Short Description: Short description can't be less than 10 charactars"
    else if (req.body.shortdesc.includes("https://discord.com")) errormsg = "Invite Link.. in the Short Description? NO!!!"
    //Long Desc Length
    else if (req.body.longdesc.length < 300) errormsg = "Long Description: Long description can't be less than 300 charactars"
    //Prefix length
    else if (req.body.prefix.length > 20) errormsg = "Prefix: prefix can't be more than 20 charactars"
    //html in short description
    else if (ishtml(req.body.shortdesc)) errormsg = "Short Description: HTML is not supported at short description!"
    //Additional owners
    else if(additionalOwners.includes(req.user.id)) errormsg = "Additional Owners: You don't need to add yourself!"
    //Check tags length
    else if (text.toString().split(",").length > 4) errormsg = "Tags : Max tags is 4"
    ...
    /*if (isjs(req.body.shortdesc)) errormsg = "Short Description: JavaScript is not supported in the short description!"*/
    else{
    if (newbot.avatar) avatar = `https://cdn.discordapp.com/avatars/${req.body.clientid}/${newbot.avatar}.png?size=256`; 
    ...
    await new BOTS({
          ...
          let r = client.guilds.cache.get(config.guildid).roles.cache.find(r => r.id === config.staff);
          let r2 = client.guilds.cache.get(config.guildid).roles.cache.find(r => r.id === config.staffTrial);
          await r.setMentionable(true)
          await r2.setMentionable(true)
          let botOwner = client.guilds.cache.get(config.guildid).members.cache.get(req.user.id)
          ...
          e.addField("Bot Page", `[View it Here](https://paradisebots.net/bots/${req.body.clientid})`)
          e.addField("Invite (Perms=0)", "[Invite](https://discord.com/oauth2/authorize?client_id=${req.body.clientid}&scope=bot&guild_id=${process.env.GUILD_ID}&permissions=0)")
          e.setTimestamp()

          var e2 = new MessageEmbed()
          e2.setTitle('Bot Submitted')
          e2.setColor('#7289DA')
          e2.setDescription('Your bot has been submitted, Please be patient we will review it as soon as possible')
          e2.setTimestamp()

          //Send embeds to user and to log channel
          await client.guilds.cache.get(config.guildid).channels.cache.get(config.botlogs).send(`${r} || ${r2}`)
          client.guilds.cache.get(config.guildid).channels.cache.get(config.botlogs).send(e)
          botOwner.send(e2)
          r.setMentionable(false)
          alertmsg = "Your bot application has been sent, be patient while we are reviewing!";
        }
        renderTemplate(res, req, "bots/add.ejs", { alert: alertmsg, error: errormsg });
})
  ...
  app.post("/reports/bug", checkAuth, async (req, res, next) => {
    ...
    if (ratelimit.has(req.user.id + "bug")) {
      ...
      if (req.body.reportbug.length > 500) {
        ...
        client.guilds.cache.get(config.guildid).channels.cache.get(config.reportslog).send(bugEmbed)
        setTimeout(() => {
          ratelimit.delete(req.user.id + "bug")
        }, 60000);
      }
    }
    renderTemplate(res, req, "reports/bug.ejs", { alert: `${alertmsg}`, error: `${errormsg}` });
  });
...
app.get("/certification", checkAuth, async (req, res, next) => {
  ...
  console.log(bots)
  renderTemplate(res, req, 'applications/certification.ejs', { alert: null, error: null, bots});
});
  // Dashboard endpoint.
  app.get("/bots/:botid/resubmit", checkAuth, async (req, res, next) => {
    ...
    BOTS.findOne({
      ...
      if (bot.owner !== req.user.id && !bot.additionalOwners.includes(req.user.id)) return res.redirect("/error")

      const botlist = await BOTS.findOne({ botid: req.params.botid }, { _id: false, auth: false })

      renderTemplate(res, req, "bots/resubmit.ejs", { bots: botlist, alert: null, error: null });
    });
...
app.post("/certification", checkAuth, async (req, res, next) => {
  ...
  if(req.body.submit){
    ...
    else if(!client.guilds.cache.get(config.guildid).members.cache.get(req.user.id)) errormsg = "You are not in the support server";
    else if(bot.owner !== req.user.id) errormsg = "You are not the owner of this bot";
    else if(bot.status !== "approved") errormsg = "This bot is at queue";
    else if(!client.guilds.cache.get(config.guildid).members.cache.get(bot.botid)) errormsg = "This bot isn't at server ask staff to add";
    else if(bot.pending_cert) errormsg = "This bot is awaiting certification";
    else if(bot.is_certified) errormsg = "This bot is already certified";
    else if(bot.certifiedBot === "certified") errormsg = "This bot is already certified";
    else if(!bot.votes >= 15) errormsg = "Your bot doesn't have enough votes.";
    else if(!bot.servers >= 75) errormsg = "Your bot is not in 75 or more servers.";
    else{
      bot.pending_cert = true;
      await bot.save();
      alertmsg = `Your bot has been added to certification queue`
      let adminRole = client.guilds.cache.get(config.guildid).roles.cache.find(r => r.id === config.web_admin);       
      await client.guilds.cache.get(config.guildid).channels.cache.get(config.botlogs).send(`${adminRole}`)
      client.guilds.cache.get(config.guildid).channels.cache.get(config.botlogs).send(`<@${req.user.id}> added <@${bot.botid}> (\`${bot.username}\`) for certification`)
    }
  }
  let bots = await BOTS.find({owner: req.user.id, status: "approved", pending_cert: false, is_certified: false}); 
  renderTemplate(res, req, 'applications/certification.ejs', { alert: alertmsg, error: errormsg, bots});
});
  //certification panel
app.post("/panel/certification", checkAuth, async (req, res, next) => {
  ...
  if(req.body.certify){
    ...
    if(!bot) errormsg = "Error occured while certification";
    else if(bot.owner === req.user.id) errormsg = "You can't certify your own bot LOL"
    else if(!client.guilds.cache.get(config.guildid).members.cache.get(bot.owner)) errormsg = "Owner not in the support server";
    else if(bot.is_certified) errormsg = "Bot it already certified!";
    else if(!client.guilds.cache.get(config.guildid).members.cache.get(bot.botid)) errormsg = "This bot isn't at server add it";
    else if(!bot.pending_cert) errormsg = "This bot is not awaiting certification";
    else if(req.user.id === bot.owner) errormsg = 'You can not certify your own bot 🤦🏻‍♂️';
    else{
      ...
      let member = client.guilds.cache.get(config.guildid).members.cache.get(bot.owner);
      let da_bot = client.guilds.cache.get(config.guildid).members.cache.get(bot.botid);
      let dev_role = client.guilds.cache.get(config.guildid).roles.cache.get(client.config.certified_devs);
      let bot_role = client.guilds.cache.get(config.guildid).roles.cache.get(client.config.certified_bots);
      await member.roles.add(dev_role);
      await da_bot.roles.add(bot_role);
      client.guilds.cache.get(config.guildid).channels.cache.get(config.botlogs).send(`<@${bot.botid}> by <@${bot.owner}> got certified by <@${req.user.id}>`)
    }
    ...
    if(!bot) errormsg = "Error occured while uncertification";
      ...
      client.guilds.cache.get(config.guildid).channels.cache.get(config.botlogs).send(`<@${bot.botid}> by <@${bot.owner}> certification got denied by <@${req.user.id}>`)
    }
  }
  let bots = await BOTS.find({owner: req.user.id, status: "approved", pending_cert: true, certified: false}); 
  renderTemplate(res, req, 'panel/certification.ejs', { alert: alertmsg, error: errormsg, certification_bots: bots});
});
 app.get("/panel/certification", checkAuth, async (req, res, next) => {
  if (!config.owners.includes(req.user.id)) return res.redirect("/")
  let bots = await BOTS.find({pending_cert: true});
  for(i = 0; i < bots.length; i++){await client.users.fetch(bots[i].botid)}
  renderTemplate(res, req, 'panel/certification.ejs', { alert: null, error: null, certification_bots: bots});
});
  ...
  app.get("/panel/admin", checkAuth, async (req, res, next) => {
   ...
   let webAdmin = req.user.id;
    ...
    if (!config.higherStaff.includes(req.user.id)) {
      ...
      renderTemplate(res, req, "panel/admin.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: null, error: null, reportbug: bugres, partners: partners, staff_apps: staff_apps });
    }
  });
...
if (req.body.checkingtransfer) {
  BOTS.findOne({ botid: req.body.clientid }, async (err, bot7) => {
    ...
    let botCheck = client.guilds.cache.get(config.guildid).members.cache.get(bot7.botid);

    let ownerCheck = client.guilds.cache.get(config.guildid).members.cache.get(req.body.userid);

    if (!botCheck) {
       return renderTemplate(res, req, "panel/admin.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: null, error: "That bot is not a member of our server", partners: partners, staff_apps: staff_apps });
    }

    if (!ownerCheck) {
       return renderTemplate(res, req, "panel/admin.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: null, error: "That user is not a member of our server.", partners: partners, staff_apps: staff_apps });
    }

    if (ownerCheck.bot) {
       return renderTemplate(res, req, "panel/admin.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: null, error: "You cannot add a bot as owner.", partners: partners, staff_apps: staff_apps });
    }

    if (req.body.userid === req.body.clientid) {
       return renderTemplate(res, req, "panel/admin.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: null, error: "Wait what!! What did you do!!! A Bot can't own a bot", partners: partners, staff_apps: staff_apps });
    }
   
     if (!newOwner) {
       return renderTemplate(res, req, "panel/admin.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: null, error: "That user does not exist in our database", partners: partners, staff_apps: staff_apps });
     }

     if (!bot7) {
       return renderTemplate(res, req, "panel/admin.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: null, error: "That bot does not exist in our database", partners: partners, staff_apps: staff_apps });
     } else {
       client.guilds.cache.get(config.guildid).channels.cache.get(config.botlogs).send(`<@${req.body.clientid}> has been transferred  by <@${req.user.id}> (<@${bot7.owner}>)\nReason: **${req.body.transferreason}**`)
       let thebot4 = client.guilds.cache.get(config.guildid).members.cache.get(req.body.clientid);
       ...
       for (i = 0; i < penbotlist.length; i++) {
         ...
         return renderTemplate(res, req, "panel/admin.ejs", { newOwer: newOwner, webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: "Successfully the bot has been transferred!", error: null, partners: partners, staff_apps: staff_apps });
    } 
  }) 
} 
      if (req.body.checkingdelete) {
        BOTS.findOne({ botid: req.body.clientid }, async (err, bot3) => {
          if (!bot3) {
            return renderTemplate(res, req, "panel/admin.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: null, error: "That bot does not exist in our database", partners: partners, staff_apps: staff_apps });
          } else {
           ...
           if(client.guilds.cache.get(config.guildid).members.cache.get(req.body.clientid)){
            ...
            return renderTemplate(res, req, "panel/admin.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: "Successfully the bot has been deleted!", error: null, partners: partners, staff_apps: staff_apps });
          }
        })
      }
      if (req.body.reject) {
        if (!client.guilds.cache.get(config.guildid).members.cache.get(req.body.reject)) {
          return renderTemplate(res, req, "panel/admin.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: null, error: "Bot is not at server!", partners: partners, staff_apps: staff_apps });
        }
        BOTS.findOne({ botid: req.body.reject }, async (err, bot) => {
          if (!bot) {
            return renderTemplate(res, req, "panel/admin.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: null, error: "Error occured while rejecting bot!", partners: partners, staff_apps: staff_apps });
          } else {
            if (bot.status == "denied") return renderTemplate(res, req, "panel/admin.ejs", { webAdmin: webAdmin, partners: partners, reportbug: bugres, bots: penbotlist, alert: null, error: "Someone already rejected the bot faster than you!", staff_apps: staff_apps });
            var deniedMsg = new MessageEmbed()
              .setTitle('Bot Denied')
              .setColor('#7289DA')
              .setDescription("Whoops, A bot has been rejected, Please make sure you read our rules before you resubmit")
            ...
            return renderTemplate(res, req, "panel/admin.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: "Success the bot has been denied!", error: null, partners: partners, staff_apps: staff_apps });
          }
        })
      }
      if (req.body.accept) {
        ...
        BOTS.findOne({ botid: req.body.accept }, async (err, bot) => {
          if (!bot) {
            ...
            if (!client.guilds.cache.get(config.guildid).members.cache.get(bot.owner)) {
              ...
              penbotlist = await BOTS.find({ status: "pending" }, { _id: false, auth: false }).sort([['descending']])
              penbotlist.filter(bot => bot)
              return renderTemplate(res, req, "panel/admin.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: null, error: "Bot owner is not at server!, bot got auto rejected", partners: partners, staff_apps: staff_apps });
            }
            if (bot.status == "approved") return renderTemplate(res, req, "panel.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: null, error: "Someone already approved the bot faster than you!", partners: partners, staff_apps: staff_apps });
            var approved = new MessageEmbed()
              .setTitle('Bot Approved')
            ...
            penbotlist = await BOTS.find({ status: "pending" }, { _id: false, auth: false }).sort([['descending']])
            penbotlist.filter(bot => bot)
            return renderTemplate(res, req, "panel/admin.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: "Success the bot has accepted!", error: null, partners: partners, staff_apps: staff_apps });
          }
        })
      }
      let alertmsg = "";
      ...
      if (req.body.acceptbug) {
        ERRORS.findOneAndDelete({ userID: req.body.acceptbug }, async (err, bug) => {
          if (!bug) {
            ...
            bugres = await ERRORS.find({}, { _id: false, auth: false })
            bugres.filter(bug => bug)
            return renderTemplate(res, req, "panel/admin.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: alertmsg, error: errormsg, partners: partners, staff_apps: staff_apps });
          } else {
            alertmsg = `Success accepted the bug (which means fixed)!`
            bugres = await ERRORS.find({}, { _id: false, auth: false })
            bugres.filter(bug => bug)
      ...
      if (req.body.accept_partner) {
        ...
        if (!user_partner) {
          ...
          partners = await partner_app.find({}, { _id: false, auth: false })
          partners.filter(app => app)
          renderTemplate(res, req, "panel/admin.ejs", { webAdmin: webAdmin, reportbug: bugres, bots: penbotlist, alert: alertmsg, error: errormsg, partners: partners, staff_apps: staff_apps });
        } else {
          alertmsg = `Successful accepted the partner!`
          await client.guilds.cache.get(config.guildid).channels.cache.get("762752184944689192").send(`<@${user_partner.userID}> (\`${user_partner.username}\`) partner app has been approved! `)
          let user = await client.users.fetch(user_partner.userID);
...
app.get("/panel/queue", checkAuth, async (req, res, next) => {
    ...
    let webAdmin = req.user.id;
     ...
     if (!config.owners.includes(req.user.id)) {
       ...
       renderTemplate(res, req, "/panel/queue.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: null, error: null, });
     }
   });
 ...
 app.post("/panel/queue", checkAuth, async (req, res, next) => {
    ...
    const webAdmin = req.user.id;
      ...
      if (req.body.checkingdelete) {
        BOTS.findOne({ botid: req.body.clientid }, async (err, bot3) => {
          if (!bot3) {
            return renderTemplate(res, req, "/panel/queue.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: null, error: "That bot does not exist in our database"});
          } else {
            client.guilds.cache.get(config.guildid).channels.cache.get(config.botlogs).send(`<@${req.body.clientid}> has been deleted by <@${req.user.id}> (<@${bot3.owner}>)\nReason: **${req.body.deletereason}**`)
            let thebot2 = client.guilds.cache.get(config.guildid).members.cache.get(req.body.clientid);
            thebot2.kick().catch(() => { })
            ...
            return renderTemplate(res, req, "/panel/queue.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: "Successfully the bot has been deleted!", error: null});
          }
        })
    }
      if (req.body.reject_bot) {
        if (!client.guilds.cache.get(config.guildid).members.cache.get(req.body.reject_bot)) {
          return renderTemplate(res, req, "/panel/queue.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: null, error: "Bot is not at server!"});
        }
        BOTS.findOne({ botid: req.body.reject_bot }, async (err, bot) => {
          if (!bot) {
            return renderTemplate(res, req, "/panel/queue.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: null, error: "Error occured while rejecting bot!"});
          } else {
            if(req.user.id == bot.owner) return renderTemplate(res, req, "/panel/queue.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: null, error: "You can't take any actions for your own bot"});
            if (bot.status == "denied") return renderTemplate(res, req, "/panel/queue.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: null, error: "Someone already rejected the bot faster than you!"});
            var deniedMsg = new MessageEmbed()
              .setTitle('Bot Denied')
              .setColor('#7289DA')
              .setDescription("Whoops, A bot has been rejected, Please make sure you read our rules before you resubmit")
            ...
            theOwner.send(deniedMsg)
            client.guilds.cache.get(config.guildid).channels.cache.get(config.botlogs).send(deniedMsg)
            let thebot = client.guilds.cache.get(config.guildid).members.cache.get(req.body.reject_bot);
            thebot.kick().catch(() => { })
            bot.status = "denied";
            bot.date = Date.now()
            await bot.save()
            ...
            return renderTemplate(res, req, "/panel/queue.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: "Success the bot has been denied!", error: null});
          }
        })
    }
      if (req.body.accept_bot) {
        if (!client.guilds.cache.get(config.guildid).members.cache.get(req.body.accept_bot)) {
          return renderTemplate(res, req, "/panel/queue.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: null, error: "Bot is not at server!"});
        }
        BOTS.findOne({ botid: req.body.accept_bot }, async (err, bot) => {
          if (!bot) {
            return renderTemplate(res, req, "/panel/queue.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: null, error: "Error occured while rejecting bot, Is it in our Database?!"});
          } else {
             if(req.user.id == bot.owner) return renderTemplate(res, req, "/panel/queue.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: null, error: "You can't take any actions for your own bot"});
            let theBotUsername = await client.guilds.cache.get(config.guildid).members.cache.get(bot.botid).username
            ...
            if (!client.guilds.cache.get(config.guildid).members.cache.get(bot.owner)) {
              ...
              client.guilds.cache.get(config.guildid).channels.cache.get(config.botlogs).send(autoDeny).catch(() => {});

              let thebot = client.guilds.cache.get(config.guildid).members.cache.get(req.body.accept_bot);
              ...
              return renderTemplate(res, req, "/panel/queue.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: null, error: "Bot owner is not at server!, bot got auto rejected"});
            }

            if (bot.status == "approved") return renderTemplate(res, req, "/panel/queue.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: null, error: "Someone already approved the bot faster than you!"});
            ...
            var approved = new MessageEmbed()
              ...
              .addField('Owner/s', `<@${bot.owner}>`)
              .addField('Feedback', `${req.body.reason}`)
              .addField("Bot Page", `[View it Here](https://paradisebots.net/bots/${bot.botid})`)
              .setTimestamp()
            ...
            penbotlist = await BOTS.find({ status: "pending" }, { _id: false, auth: false }).sort([['descending']])
            penbotlist.filter(bot => bot)
            return renderTemplate(res, req, "/panel/queue.ejs", { webAdmin: webAdmin, bots: penbotlist, alert: "Success the bot has been accepted!", error: null});
             }
   })
  }
});
...
app.get("/panel/reports", checkAuth, async (req, res, next) => {
   ...
   let webAdmin = req.user.id;
    ...
    if (!config.owners.includes(req.user.id)) {
      ...
      renderTemplate(res, req, "panel/reports.ejs", { webAdmin: webAdmin, alert: null, error: null, reportbug: bugres });
    }
  });
      ...
      if (req.body.acceptbug) {
        ERRORS.findOneAndDelete({ userID: req.body.acceptbug }, async (err, bug) => {
          if (!bug) {
            ...
            bugres = await ERRORS.find({}, { _id: false, auth: false })
            bugres.filter(bug => bug)
            return renderTemplate(res, req, "panel/reports.ejs", { webAdmin: webAdmin, reportbug: bugres, alert: alertmsg, error: errormsg });
          } else {
            alertmsg = `Bug has been marked as Fixed and Removed!`
            bugres = await ERRORS.find({}, { _id: false, auth: false })
            bugres.filter(bug => bug)         

            client.guilds.cache.get(config.guildid).channels.cache.get(config.botlogs).send(bugEmbed2)
            client.guilds.cache.get(config.guildid).channels.cache.get(config.reportslog).send(bugEmbed2)
           ...
           let theActualUser = client.users.cache.get(req.body.acceptbug).username
...
// Resubmit Bot Page
    ...
    BOTS.findOne({
      ...
      if (!bot) {
        ...
        if (req.body.shortdesc.length > 150) {
          return renderTemplate(res, req, "bots/resubmit.ejs", { bots: botlist, alert: null, error: "Short Description: Short description can't be more than 150 charactars" });
        } else if (req.body.shortdesc.length < 10) {
          return renderTemplate(res, req, "bots/resubmit.ejs", { bots: botlist, alert: null, error: "Short Description: Short description can't be less than 10 charactars" });
        } else if (req.body.shortdesc.includes("https://discord.com")) {
          return renderTemplate(res, req, "bots/resubmit.ejs", { bots: botlist, alert: null, error: "Invite Link.. in the Short Description? NO!!!" });
        }

        if (req.body.longdesc.length < 300) {
          return renderTemplate(res, req, "bots/resubmit.ejs", { bots: botlist, alert: null, error: "Long Description: Long description can't be less than 300 charactars" });
        }

        if (req.body.prefix.length > 20) {
          return renderTemplate(res, req, "bots/resubmit.ejs", { bots: botlist, alert: null, error: "Prefix: prefix can't be more than 20 charactars" });
        }

        if (ishtml(req.body.shortdesc)) {
          return renderTemplate(res, req, "bots/resubmit.ejs", { bots: botlist, alert: null, error: "Short Description: HTML is not supported in the short description!" });
        }
        ...
        if (Array.isArray(text) == true) {
          tags2 = text.join(", ")
          if (text.length > 4) return renderTemplate(res, req, "bots/resubmit.ejs", { bots: botlist, alert: null, error: "Tags : Max tags is 4" });
        } else {
          tags2 = text
        }
        let additionalOwners = req.body.additionals.split(" ");
        ...
        var e2 = new MessageEmbed()
          ...
          .addField("Bot Page", `[View it Here](https://paradisebots.net/bots/${req.body.clientid})`)
          .setTimestamp()
        await client.guilds.cache.get(config.guildid).channels.cache.get(config.botlogs).send(`${r}`)
        client.guilds.cache.get(config.guildid).channels.cache.get(config.botlogs).send(e2)
        r.setMentionable(false);
        await renderTemplate(res, req, "bots/resubmit.ejs", { bots: botlist, alert: "Success your bot is waiting at pending bots queue!", error: null })
      }
    });
  });
  app.get("/bots/:id/widget", async (req, res, next) => {
  let bot = await BOTS.findOne({botid: req.params.id})
  if(!bot) bot = await BOTS.findOne({vanity: req.params.id, is_certified: true})
  if (!bot) return res.redirect("/error")
  try {
    registerFont(path.join(__dirname, 'public/fonts/quicksand/static/Quicksand-Bold.ttf'), { family: 'Quicksand' })
    let fetch = await client.users.fetch(bot.botid).catch(() => {});
    let owner = await client.users.fetch(bot.owner).catch(() => {});
    let bot_avatar = decodeURIComponent(fetch.displayAvatarURL().replace("/avatar/?avatar=", "")).replace(".png", ".png?size=512").replace(".webp", ".png?size=512");
    ...
    .printText(`${bot.shards === 0 ? "N/A" : formatNumbers(bot.shards)} Shards`, 140, 137)
    .printText(bot.library, 260, 137)
    .printText(`${fetch.presence.status.toUpperCase()}`, 20, 107)
    .printText(`${bot.votes === 0 ? 0 : formatNumbers(bot.votes)} Votes`, 105, 107)
    .printText(formatNumbers(bot.likes.size), 225, 107)
    .printText(formatNumbers(bot.dislikes.size), 310, 107)
    .setTextSize(23)
    .printText(client.users.cache.get(bot.botid) ? client.users.cache.get(bot.botid).username.length > 25 ? client.users.cache.get(bot.botid).username.slice(0, 25) + "..." : client.users.cache.get(bot.botid).username : bot.username.length > 25 ? bot.username.slice(0, 25) + "..." : bot.username, 75, 40)
    .setTextSize(13)
    ...
    if(bot.is_certified){
      image
      .setTextFont("8px Quicksand")
      .printText("CERTIFIED", 75, 62)
    }
  ...
  app.get("/bots/:botid", async (req, res, next) => {
    ...
    if(!bot) bot = await BOTS.findOne({vanity: req.params.botid, is_certified: true});
    if (!bot) return res.redirect("/error")
    if(req.user && client.config.owners.includes(req.user.id) || req.user && bot.owner === req.user.id) {
    ...
    if(!bot) bot = await BOTS.findOne({vanity: req.params.botid, status: "approved", is_certified: true});
    if(!bot) return res.redirect("/error");
    if(req.params.id == "none") return res.redirect("/error");
    }
    ...
    return res.redirect(`/bots/${bot.vanity}`)
      ...
      let certifiedBot = await BOTS.findOne({ botid: botlist.botid }, { certifiedBot: 'certified' }, { _id: false, auth: false })
      let partneredBot = await BOTS.findOne({ botid: botlist.botid }, { partneredBot: 'partnered' }, { _id: false, auth: false })
      if(!certifiedBot) certifiedBot = await BOTS.findOne({ vanity: req.params.botid }, { certifiedBot: 'certified' }, { _id: false, auth: false })
      if(!partneredBot) partneredBot = await BOTS.findOne({ vanity: req.params.botid }, { partneredBot: 'partnered' }, { _id: false, auth: false })
      let data = {
        isPartneredBot: partneredBot.partneredBot === 'partnered' ? true : false,
        isCertifiedBot: certifiedBot.certifiedBot === 'certified' ? true : false,
        desc: desc,
  ...
  app.post("/bots/:botid", checkAuth, async (req, res, next) => {
    ...
    let theBot = await BOTS.findOne({ botid: req.params.botid }, { _id: false, auth: false })
    let certifiedBot = await BOTS.findOne({ botid: req.params.botid }, { certifiedBot: 'certified' }, { _id: false, auth: false })
    let partneredBot = await BOTS.findOne({ botid: req.params.botid }, { partneredBot: 'partnered' }, { _id: false, auth: false })
    ...
    const botOwner = await client.users.fetch(theBot.owner);
     // if (bot.status == "denied") return res.redirect("/error")
    if(!certifiedBot) certifiedBot = await BOTS.findOne({ vanity: req.params.botid }, { certifiedBot: 'certified', is_certified: true}, { _id: false, auth: false })
    if(!partneredBot) partneredBot = await BOTS.findOne({ vanity: req.params.botid }, { partneredBot: 'partnered' }, { _id: false, auth: false })
      var desc = ``;
      let isUrl = url2(bot.long.replace("\n", "").replace(" ", ""))
      ...
      if (req.body.like) {
       ...
       await bot.save()
       return renderTemplate(res, req, "bots/botpage.ejs", { owners: additionalOwnersArray, isCertifiedBot: certifiedBot.certifiedBot === 'certified' ? true : false, isPartneredBot: partneredBot.partneredBot === 'partnered' ? true : false, desc: desc, isURL: isUrl, bots: botlist, alert: null, error: null });
      }
     if (req.body.dislike) {
      ...
      if (req.body.token) {
        ...
        if (bot.owner !== req.user.id) return renderTemplate(res, req, "bots/botpage.ejs", data);
        return renderTemplate(res, req, "bots/botpage.ejs", { owners: additionalOwnersArray, isCertifiedBot: certifiedBot.certifiedBot === 'certified' ? true : false, isPartneredBot: partneredBot.partneredBot === 'partnered' ? true : false, desc: desc, isURL: isUrl, bots: botlist, alert: `Authorization Token: ${bot.auth}`, error: null });
      }
      if (req.body.newtoken) {
        if (!req.user) return res.redirect("/login")
        if (bot.owner !== req.user.id) return renderTemplate(res, req, "bots/botpage.ejs", { owners: additionalOwnersArray, certifiedBot: bot.certifiedBot, desc: desc, isURL: isUrl, bots: botlist, alert: null, error: null });
        ...
        await bot.save()
        botlist = await BOTS.findOne({ botid: req.params.botid }, { _id: false, auth: false })
        return renderTemplate(res, req, "bots/botpage.ejs", { owners: additionalOwnersArray, isCertifiedBot: certifiedBot.certifiedBot === 'certified' ? true : false, isPartneredBot: partneredBot.partneredBot === 'partnered' ? true : false, desc: desc, isURL: isUrl, bots: bot, alert: `New Authorization Token: ${bot.auth}`, error: null });
      }
      if (req.body.permdelete) {
        if (!req.user) return res.redirect("/login")
        if (bot.owner !== req.user.id) return renderTemplate(res, req, "bots/botpage.ejs", { owners: additionalOwnersArray, isCertifiedBot: certifiedBot.certifiedBot === 'certified' ? true : false, isPartneredBot: partneredBot.partneredBot === 'partnered' ? true : false, desc: desc, isURL: isUrl, bots: botlist, alert: null, error: null });
        await BOTS.findOne({ botid: req.params.botid }, async (err, bot2) => {
          ...
          var e4 = new MessageEmbed()
            ...
            .setTimestamp()
          client.guilds.cache.get(config.guildid).channels.cache.get(config.botlogs).send(e4)
          ...
          const botlist = await BOTS.find({ status: "approved" }).sort([['votes', 'descending']])
          botlist.filter(bot => bot)
          const botlist2 = await BOTS.find({ status: "approved" }).sort([['date', 'descending']])
          botlist2.filter(bot => bot)
          for (i = 0; i < botlist.length; i++) {
            await client.users.fetch(botlist[i].botid)
          }
...
app.get("/bots/:botid/report", checkAuth, async (req, res, next) => {
    let theBot = await BOTS.findOne({ botid: req.params.botid, status: "approved" })
    if(!theBot) theBot = await BOTS.findOne({ vanity: req.params.botid, status: "approved", is_certified: true })
    ...
    const fetch = await client.users.fetch(theBot.botid).catch(() => {});
     renderTemplate(res, req, "reports/reportbot.ejs", {bots: fetch, alert: null, error: null});
   })
  app.post("/bots/:botid/report", checkAuth, async (req, res, next) => {
    ...
    if(req.body.report_bot){
      let e = new MessageEmbed()
       .setAuthor(`${fetch.username} has been Reported by ${req.user.username}`, fetch.displayAvatarURL())
       .setColor('#7289DA')
       .addField("Bot", `<@${fetch.id}> (\`${fetch.username}\`), ID: (\`${fetch.id}\`)`)
       .addField("Bot Owner", `<@${owner_fetch.id}> (\`${owner_fetch.username}\`), ID: (\`${owner_fetch.id}\`)`)
       .addField("Reporter", `<@${req.user.id}> (\`${req.user.username}\`), ID: (\`${req.user.id}\`)`)
       .addField("Report Reason", req.body.report_desc)
       .setTimestamp()
    guild.channels.cache.get(client.config.bot_reportslog).send(e).then(msg => {
    msg.react("✅")
    msg.react("❌")
    })
      alert = "Your report has been sent!"
    }
     renderTemplate(res, req, "reports/reportbot.ejs", {bots: fetch, alert, error});
   })
app.get("/bots/:botid/vote", checkAuth, async (req, res, next) => {
    ...
    const fetch = await client.users.fetch(theBot.botid).catch(() => {});
     renderTemplate(res, req, "bots/vote.ejs", {bots: fetch, alert: null, error: null});
   })
app.post("/bots/:botid/vote", checkAuth, async (req, res, next) => {
    ...
    if (req.body.vote) {
         ...
         let check22 = await VOTES.findOne({ user: req.user.id, clientid: theBot.botid });
          if (theAbuser && theAbuser.voteBanned === "banned") {
           let data = {
             bots: fetch,
             alert: null,
             error: "You have been banned from Voting due to abuse of our API, Contact Us now: https://paradisebots.net/join"
           }
           ...
           let data = {
             bots: fetch,
             alert: null,
             error: "This bot has been flagged for recieving votes to quickly, If you're the bots owner please ontact Us now: appeals@paradisebots.net"
           }
           return renderTemplate(res, req, "bots/vote.ejs", data);
         }
         if (!check22) {
           ...
           var e3 = new MessageEmbed()
              ...
              .addField("Bot Page", `[View it Here](https://paradisebots.net/bots/${req.params.botid})`)
             .setThumbnail(botAvatar)
             .setTimestamp()
           
            client.guilds.cache.get(config.guildid).channels.cache.get(config.votelogs).send(e3)       
           ...
           let webhookClient;
           if(webhook[3] && webhook[4]) webhookClient = new WebhookClient(webhook[3].toString(), webhook[4].toString());
    ...
    BOTS.findOne({ botid: req.params.botid, }, async (err, bot) => {
     ...
     let isUrl = url2(bot.long.replace("\n", "").replace(" ", ""))
      ...
      if (!bot) {
        ...
        if (bot.owner !== req.user.id && !bot.additionalOwners.includes(req.user.id)) return res.redirect("/")

        let botlist = await BOTS.findOne({ botid: req.params.botid }, { _id: false, auth: false })
        botlist = await BOTS.findOne({ botid: req.params.botid }, { _id: false, auth: false })

        renderTemplate(res, req, "bots/editbot.ejs", { isURL: isUrl, bots: botlist, alert: null, error: null })
      }
    });
  });
  ...
  app.post("/bots/:botid/edit", checkAuth, async (req, res, next) => {
    BOTS.findOne({ botid: req.params.botid, }, async (err, bot) => {
      ...
      if (!bot) {
        ...
        if (req.body.shortdesc.length > 150) {
        let isUrl = url2(req.body.longdesc.replace("\n", "").replace(" ", ""))
          return renderTemplate(res, req, "bots/editbot.ejs", { isURL: isUrl, bots: botlist, alert: null, error: "Short Description: Short description can't be more than 150 charactars" });
        } else if (req.body.shortdesc.length < 10) {
        let isUrl = url2(req.body.longdesc.replace("\n", "").replace(" ", ""))
          return renderTemplate(res, req, "bots/editbot.ejs", { isURL: isUrl, bots: botlist, alert: null, error: "Short Description: Short description can't be less than 10 charactars" });
        } else if (req.body.shortdesc.includes("https://discord.com")) {
        let isUrl = url2(req.body.longdesc.replace("\n", "").replace(" ", ""))
          return renderTemplate(res, req, "bots/editbot.ejs", { isURL: isUrl, bots: botlist, alert: null, error: "Invite Link.. in the Short Description? NO!!!" });
        }

        if (req.body.longdesc.length < 300) {
        let isUrl = url2(req.body.longdesc.replace("\n", "").replace(" ", ""))
          return renderTemplate(res, req, "bots/editbot.ejs", { isURL: isUrl, bots: botlist, alert: null, error: "Long Description: Long description can't be less than 300 charactars" });
        }

        if (req.body.prefix.length > 20) {
        let isUrl = url2(req.body.longdesc.replace("\n", "").replace(" ", ""))
          return renderTemplate(res, req, "bots/editbot.ejs", { isURL: isUrl, bots: botlist, alert: null, error: "Prefix: prefix can't be more than 20 charactars" });
        }


        if (ishtml(req.body.shortdesc)) {
        let isUrl = url2(req.body.longdesc.replace("\n", "").replace(" ", ""))
          return renderTemplate(res, req, "bots/editbot.ejs", { isURL: isUrl, bots: botlist, alert: null, error: "Short Description: HTML is not supported at short description!" });
        }
       ...
       if (botlist.avatar) {
        ...
        let isUrl = url2(req.body.longdesc.replace("\n", "").replace(" ", ""))
          ...
          if (text.length > 4) return renderTemplate(res, req, "bots/editbot.ejs", { isURL: isUrl, bots: botlist, alert: null, error: "Tags : Max tags is 4" });
        ...
        if(req.body.additionals){
          ...
          let isUrl = url2(req.body.longdesc.replace("\n", "").replace(" ", ""))
            return renderTemplate(res, req, "bots/editbot.ejs", { isURL: isUrl, bots: botlist, alert: null, error: "Additional Owners: You don't need to add yourself!" });
          } 
          ...
          for (i of additionalOwners) {
            if (!guild.members.cache.get(i)) {
             let isUrl = url2(req.body.longdesc.replace("\n", "").replace(" ", ""))
              return renderTemplate(res, req, "bots/editbot.ejs", { isURL: isUrl, bots: botlist, alert: null, error: "The additional owners are not in our sever, ask them to join and try again!" });
            }
          }
        }
        ...
        client.guilds.cache.get(config.guildid).channels.cache.get(config.botlogs).send(e4)
        r.setMentionable(false)
        await renderTemplate(res, req, "bots/editbot.ejs", { isURL: isUrl, bots: botlist, alert: "Success your bot has been updated!", error: null })
      }
    });
  });
  ...
  app.get("/bots", async (req, res, next) => {
    const certifiedBot = await BOTS.find({ status: "approved" }, { certifiedBot: 'certified' }, { _id: false, auth: false })
    const botlist = await BOTS.find({ status: "approved" }, { _id: false, auth: false }).sort([['votes', 'descending']])
    botlist.filter(bot => bot)
    for (i = 0; i < botlist.length; i++) {
      await client.users.fetch(botlist[i].botid)
    }
    renderTemplate(res, req, "bots/bots.ejs", { isCertifiedBot: certifiedBot.certifiedBot === 'certified' ? true : false, bots: botlist, alert: null, error: null });
  });
  ...
  app.get("/certified", async (req, res, next) => {
    const certifiedList = await BOTS.find({ status: "approved", certifiedBot: "certified" }, { _id: false, auth: false }).sort([['votes', 'descending']])
    certifiedList.filter(bot => bot)
    for (i = 0; i < certifiedList.length; i++) {
      await client.users.fetch(certifiedList[i].botid)
    }
    renderTemplate(res, req, "bots/certified.ejs", { bots: certifiedList, alert: null, error: null });
  });
  ...
  app.get("/partnered", async (req, res, next) => {
    const partneredList = await BOTS.find({ status: "approved", partneredBot: "partnered" }, { _id: false, auth: false }).sort([['votes', 'descending']])
    partneredList.filter(bot => bot)
    for (i = 0; i < partneredList.length; i++) {
      await client.users.fetch(partneredList[i].botid)
      const isPartnered = await BOTS.find({ botid: partneredList[i].botid }, { _id: false, auth: false });
    }
    renderTemplate(res, req, "bots/partnered.ejs", { bots: partneredList, alert: null, error: null });
  });
  ...
  app.get("/profile", checkAuth, async (req, res, next) => {
    ...
    renderTemplate(res, req, "users/profile.ejs", { bots: userbots, bots2: otherBots, profile: users, alert: null, error: null });
  ...
  app.post("/profile", checkAuth, async (req, res, next) => {
    ...
    if (req.body.changebio) {
      if (req.body.changebio.length > 50) {
        ...
        return renderTemplate(res, req, "users/profile.ejs", { bots: userbots, bots2: otherBots, profile: users, alert: alertmsg, error: errormsg });
    ...
    if (req.body.captcha == "delete") {
      if (req.body.requestdelete) {
        if (!users) {
          ...
          users = await USERS.findOne({ userID: req.user.id })
          return await renderTemplate(res, req, "users/profile.ejs", { bots: userbots, bots2: otherBots, profile: users, alert: alertmsg, error: errormsg });
        }
      }
    } else {
    ...
    renderTemplate(res, req, "users/profile.ejs", { bots: userbots, bots2: otherBots, profile: users, alert: alertmsg, error: errormsg });
  });
...
// TRANSFER BOT OWNER STUFF HERE
  app.get("/bots/:botid/transfer", checkAuth, async (req, res, next) => {
    ...
    renderTemplate(res, req, "bots/transfer-bot.ejs", { bots: bot, alert: null, error: null });
  ...
  app.post("/bots/:botid/transfer", checkAuth, async (req, res, next) => {
    ...
    let newOwner = newOwner = await USERS.findOne({ userID: req.body.userid });
    ...
    if (req.body.checkingtransfer) {
      ...
      if (!newOwner) {
         ...
         return renderTemplate(res, req, "bots/transfer-bot.ejs", { newOwner: newOwner, bots: userbots, alert: alertmsg, error: errormsg });
      }
      if (!bot7) {

...
module.exports.run = async (client , message, args) => {
    ...
    let member = message.mentions.members.first() || client.users.cache.get(args[0]);
     ...
     if (!member || member.bot) return message.channel.send(`Ping a **user** to get info about.`);
     ...
     let userBots = await BOTS.find({ owner: member.user.id}, { _id: false })
      ...
      if(member) {
        let embed = new MessageEmbed()
          ...
          .setDescription(`Assigned Roles: ${member.roles.cache.map(role => role.toString()).join(' ')}`)

...
module.exports.run = async (client, message, args) => {
  ...
  try {
    ...
    if (!args[0])
      return message.channel.send('Ping a user or provide a User ID');
    ...
    let bots = await BOTS.find(
      { owner: user.id, status: 'approved' },
      { _id: false },
    );
    let bots2 = await BOTS.find(
      { owner: user.id, status: 'pending' },
      { _id: false },
    );
    let bots3 = await BOTS.find(
      { owner: user.id, status: 'denied' },
      { _id: false },
    );
    let bots4 = await BOTS.find(
      { additionalOwners: user.id, status: 'approved' },
      { _id: false },
    );
...
module.exports.help = {
  ...
  aliases: ['user-bots'],
  description:
    'Provides some information about what bots the provided user owns or has been added to as an Additional Owner',
  example: '``botinfo <@bot>``',
};

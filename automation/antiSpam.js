...
client. on('message', async (message) => {
  ...
  if (result === true) {
   ...
   let logEmbed = new MessageEmbed()
       ...
       logEmbed.addField("Moderator", client.user.username + " (ID: " + client.user.id + ")")
       logEmbed.addField("User", message.author.username + " (ID: " + message.author.id + ")")
       ...
       const modLogs = await client.guilds.cache.get(guildID).channels.cache.find(c => c.name === 'mod-logs').send(logEmbed)
   ...
   let noSpam = new MessageEmbed()
       ...
       noSpam.setDescription(`<@${message.author.id}> Quit spamming nonsense in my server!!!!`)
       noSpam.setTimestamp() 

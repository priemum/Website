...
message.delete().catch()
   ...
   let theUser = await user_warns.findOne({ userID: warnedUser.id });
       ...
       if (!theUser) return message.reply('The user you provided has never been warned in my database or the warnings were purged by a mod.');

       if (!theUser.numberOfWarns) return message.reply('The user you provided has never been warned in my database.');

       if (theUser.numberOfWarns === null) return message.reply('The user you provided doesnt have any warnings or an error has occurred.');
       ...
       /*if (theUser.recentlyCleared === true) 
        ...
        message.channel.send(new MessageEmbed()
            .setTitle(`Warns | ${warnedUser.username}`)
            .setThumbnail(userAvatar)
            .setDescription('Below is some info on this users most recent warn, along with a total count.')
            .setColor('#7289DA')

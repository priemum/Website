...
function checkForSpam(userID) {
  ...
  if (counter[userID] > actionsLimit) {
    ...
    client.guilds.cache.get(guildID).members.cache.get(userID).roles.add(cooldownRoleID).then (
      console.log(userID + "is on cooldown!")
    )

    client.guilds.cache.get(guildID).members.cache.get(userID).roles.remove(staffRoleID).then (

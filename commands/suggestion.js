...
module.exports.run = async (client , message, args) => {
   ...
   let channel = message.guild.channels.cache.find(channel => channel.name === 'suggestions');
    ...
    const noPerms = new MessageEmbed()
        ...
        .setDescription('Sorry, i dont have the perms to do that i need ``ADD_REACTIONS``.')
        .setFooter("Suggestion by " + message.author.username, message.author.avatarURL)
        .setTimestamp()

    if (!message.guild.member(client.user).hasPermission('ADD_REACTIONS')) return message.reply(noPerms)
...
module.exports.help = {
    ...
    category: "info",
    aliases: ['suggestion'],
    description: "Make a Suggestion for Paradise Bot List and have it Auto sent to the Suggestions Channel.",

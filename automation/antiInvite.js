...
client.on("message", async message =>{
  ...
  if (message.channel.name === '┊partner-ads' || message.channel.name === '┊self-promo') return;
   ...
   if (message.member.hasPermission("ADMINISTRATOR")) return;  
    ...
    const modLogs = message.guild.channels.cache.find(channel => channel.name === 'mod-logs');
     ...
     let logEmbed = new MessageEmbed()
         ...
         logEmbed.addField("Reason", "Advertising or Sharing Discord/Discord Server links outside of our Partnership Program.")
         logEmbed.setTimestamp()

     let userEmbed = new MessageEmbed()
         userEmbed.setTitle("LOL Nice Try Though")
         userEmbed.setDescription(`You have been muted`)
         userEmbed.addField("Reason", "Advertising of Server Links is allowed by Partnered members only")

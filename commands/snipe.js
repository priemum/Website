...
String.prototype.embedify = function() {
    ...
    if(args[0] == 'image') {
        ...
        console.log(image[1])
        return message.channel.send(new MessageEmbed().setColor("#7289DA").setImage(image[1]))
    }

    let counter = 0

    return message.channel.send(`${snipe.map(msg => `**${++counter} -** ${msg[0].content ? `${msg[0].content}${!msg[1] ? '' : '\n[IMAGE WAS DELETED]'}` : (!msg[1] ? '' : '[IMAGE WAS DELETED]')}\n**Author -** <@${msg[0].author.id}>\n**Channel -** <#${msg[0].channel.id}>`).join('\n\n')}`.embedify().addField('NOTE:', `Messages appear in order, newest deleted message is \`1.\` ,etc. Only the last five deleted messages are preserved. Messages above a 200 character limit are truncated to fit within the embed.`))
